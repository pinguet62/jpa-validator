...
public class IdentityGeneratedIdValidator extends AbstractValidator {

    public IdentityGeneratedIdValidator(Class<?> entity, String tableName) {
    ...
    protected boolean doProcess(Field field) {
        ...
        // Auto-increment: generated by: identity
        if (!JdbcMetadataChecker.INSTANCE.checkAutoIncrementByIdentity(tableName, columnName)) {
            throwError(format("column is not 'generated as identity': %s.%s & sequenceName", tableName, columnName));
            return false;
        }
        if (JdbcMetadataChecker.INSTANCE.checkIsAutoIncrementByAnySequence(tableName, columnName)) {
            throwError(format("column is 'generated as sequence': %s.%s & sequenceName", tableName, columnName));
            return false;
        }
    ...
    public Collection<Class<? extends Annotation>> getSupportedAnnotations() {
        return asList(Id.class, Column.class, GeneratedValue.class);
    }

    @Override
    public boolean support(Field field) {
        return field.isAnnotationPresent(Id.class) && field.isAnnotationPresent(GeneratedValue.class)
                && field.getDeclaredAnnotation(GeneratedValue.class).strategy().equals(IDENTITY);
    }
